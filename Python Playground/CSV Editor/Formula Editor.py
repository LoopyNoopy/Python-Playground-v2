...
def levelTestFormula():
    ...
    for row in csvFile:
        ...
        if row[1] == "Pass" or row[1] == "Fail" or row[1] == "N/A" or row[1] == "To Test":
            row[1] = '=IFS(INDIRECT("Setup!B{}")=True,"Pass",INDIRECT("Setup!C{}")=True,"Fail",INDIRECT("Setup!D{}")=True,"N/A",INDIRECT("Setup!E{}")=True,"Retest")'.format(count+2,count+2,count+2,count+2)
    count=0
    for row in csvFile:
        count = count+1
        if row[2] == "Pass" or row[2] == "Fail" or row[2] == "N/A" or row[2] == "To Test":
            row[2] = '=IFS(INDIRECT("Setup!F{}")=True,"Pass",INDIRECT("Setup!G{}")=True,"Fail",INDIRECT("Setup!H{}")=True,"N/A",INDIRECT("Setup!I{}")=True,"Retest")'.format(count+2,count+2,count+2,count+2)
...
def setupFormula():
    ...
    for row in levelFile:
        ...
        if row[1] == "Pass" or row[1] == "Fail" or row[1] == "N/A" or row[1] == "To Test":
            ...
            row[4] = "=NOT(OR(C{}=True,B{}=True,A{}=True))".format(count, count, count)
            #Mac
